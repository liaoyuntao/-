$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'lyuser/localaudit/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '申请时间', name: 'acTime', index: 'ac_time', width: 80 },
			...
			{ label: '审核状态', name: 'audit', index: 'audit', width: 60, formatter: function(value, options, row){
				return value === 0 ? 
						'<span class="label label-danger">待审核</span>' : 
						'<span class="label label-success">已拒绝</span>';
				} 
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.lyUser.userId == null ? "lyuser/save" : "lyuser/localaudit";
			$.ajax({
				type: "POST",
			    url: baseURL + url,

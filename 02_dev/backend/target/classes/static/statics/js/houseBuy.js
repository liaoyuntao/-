...
var vm = new Vue({
    ...
    methods: {
        ...
        conmit: function () {
            ...
            var regPos = /^\d+(\.\d+)?$/;
            if (!regPos.test(vm.demand.buildingAge)) {
                alert("请输入正确格式的楼龄！");
                return
            }
            ...
            console.log(vm.demand.region)
            if (vm.demand.region == null || vm.demand.region == '') {
                alert("区域要求不能为空！");
                return
            }

            vm.demand.demandType = '1';
            ...
            console.log(that.request.openid);
        ...
        init: function () {
            ...
            AjaxLoadingGetJson("/xjdc/generatorbusconfig/querySysBusConfigList", {}, function (r) {
                if (r.code == 0) {
                    ...
                    that.lists.list3 = that.busconfig.xjdcdemanddecoration.list
                    that.lists.list4 = that.busconfig.xjdcdemandpaytype.list

        ...
        methods: {
            ...
            init (id) {
                ...
                this.$nextTick(() => {
                    ...
                    if (id) {
                        API.sysdictionaryitem.info(id).then(({data}) => {
                            if (data && data.code === 0) {
            ...
            dataFormSubmit () {
                this.$refs['dataForm'].validate((valid) => {
                    ...
                    'createMac': this.dataForm.createMac,
                    'updateMac': this.dataForm.updateMac,
                    'createIp': this.dataForm.createIp,
                    'updateIp': this.dataForm.updateIp,
                    'versionNo': this.dataForm.versionNo,
                    'sort': this.dataForm.sort,                }
                    var tick = !this.dataForm.id ? API.sysdictionaryitem.add(params) : API.sysdictionaryitem.update(params)
                    tick.then(({data}) => {
                        if (data && data.code === 0) {
                            this.$message({
                                    message: '操作成功',

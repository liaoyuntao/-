<template>
    <el-dialog
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
                    ...
                    <el-form-item label="协议" prop="agreementId">
                                                            <el-input v-model="dataForm.agreementId" placeholder="协议"></el-input>
        ...
        methods: {
            init (id) {
                ...
                this.$nextTick(() => {
                    ...
                    if (this.dataForm.id) {
                        API.sysprotocolhistory.info(this.dataForm.id).then(({data}) => {
                            if (data && data.code === 0) {
                                        ...
                                        this.dataForm.agreementId = data.data.agreementId
                            }
                        })
                    }
                })
            ...
            dataFormSubmit () {
                this.$refs['dataForm'].validate((valid) => {
                    ...
                    'agreementId': this.dataForm.agreementId,
                }
                    var tick = !this.dataForm.id ? API.sysprotocolhistory.add(params) : API.sysprotocolhistory.update(params)
                    tick.then(({data}) => {
                        if (data && data.code === 0) {
                            this.$message({
                                    message: '操作成功',

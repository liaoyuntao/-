<template>
    <el-dialog
            :title="写日志" :modal-append-to-body="false"
        ...
        methods: {
            ...
            init (id) {
                ...
                this.$nextTick(() => {
                    ...
                    if (id) {
                        API.oaemaildrafts.info(id).then(({data}) => {
                            if (data && data.code === 0) {
              ...
              this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                  var params = {
                    ...
                    'receiverNo': this.dataForm.receiverNo,
                    'sender': this.dataForm.sender,
                    'senderNo': this.dataForm.senderNo,
                    'sendTime': this.dataForm.sendTime,
                    'theme': this.dataForm.theme,
                    'content': this.dataForm.content,
                    'type': this.dataForm.type,
                    'state': this.dataForm.state,
                    'isReceipt': this.dataForm.isReceipt,
                    'relevanceMainline': this.dataForm.relevanceMainline,
                    'fileName': this.dataForm.fileName,
                    'filePath': this.dataForm.filePath
                  }
                  var tick = API.oaemaildrafts.add(params)
                  tick.then(({data}) => {
                    if (data && data.code === 0) {
                      this.$message({
                        message: '操作成功',

  ...
  <div class="mod-generator">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('yczj:generator:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除
        </el-button>
      </el-form-item>
    </el-form>
    <el-table
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        API.generator.list(params).then(({data}) => {
          if (data && data.code === 0) {
      ...
      generate (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '生成' : '批量生成'}]操作?`, '提示', {
          ...
          location.href = self.url + '/sys/generator/code?tables=' + JSON.stringify(ids)
        })
      },
      // 删除
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          API.generator.del(ids).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',

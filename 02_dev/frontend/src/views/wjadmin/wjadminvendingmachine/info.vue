    ...
    <el-dialog
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
                    ...
                    <el-form-item label="机器编号" prop="number">
                                                            <el-input v-model="dataForm.number" placeholder="机器编号"></el-input>
                    ...
                    <el-form-item label="补货人授权" prop="replenishmentPersonId">
                                                            <el-input v-model="dataForm.replenishmentPersonId" placeholder="补货人授权"></el-input>
    ...
    export default {
        data () {
            ...
            dataRule: {
                        ...
                        replenishmentPersonId: [
                        { required: true, message: '补货人授权不能为空', trigger: 'blur' }
                    ],
            }
        }
        },
        methods: {
            init (id) {
                ...
                this.$nextTick(() => {
                    ...
                    if (this.dataForm.id) {
                        API.wjadminvendingmachine.info(this.dataForm.id).then(({data}) => {
                            if (data && data.code === 0) {
                                        ...
                                        this.dataForm.replenishmentPersonId = data.data.replenishmentPersonId
                            }
                        })
                    }
                })
            ...
            dataFormSubmit () {
                this.$refs['dataForm'].validate((valid) => {
                    ...
                    'replenishmentPersonId': this.dataForm.replenishmentPersonId,
                }
                    var tick = !this.dataForm.id ? API.wjadminvendingmachine.add(params) : API.wjadminvendingmachine.update(params)
                    tick.then(({data}) => {
                        if (data && data.code === 0) {
                            this.$message({
                                    message: '操作成功',

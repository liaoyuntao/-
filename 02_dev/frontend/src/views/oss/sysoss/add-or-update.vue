    ...
    <el-dialog
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
            <el-form-item :label="item.pageComment" :prop="item.fieldName" v-for="(item,indexs) in tableFieldMap[pathUrl].isSetList">
                ...
                <!--日期date-->
                <el-date-picker v-model="dataForm[item.fieldName]"  :placeholder="item.pageComment" type="date" style="width:100%;"  v-else-if="item.inputType==='2'"></el-date-picker>
                <!--时间datetime-->
                <el-date-picker v-model="dataForm[item.fieldName]"  :placeholder="item.pageComment" type="datetime" style="width:100%;"  v-else-if="item.inputType==='3'"></el-date-picker>
                <!--下拉选select-->
                <el-select v-model="dataForm[item.fieldName]" placeholder="请选择" style="width:100%;" v-else-if="item.inputType=='4'">
                    <el-option
                ...
                <!--富文本框-->
                <el-input v-model="dataForm[item.fieldName]" :placeholder="item.pageComment" v-else-if="item.inputType=='6'"></el-input>
                <!--文本域-->
        ...
        dataFormSubmit () {
          this.$refs['dataForm'].validate((valid) => {
            if (valid) {
              ...
              var tick = !this.dataForm.id ? API.sysoss.add(params) : API.sysoss.update(params)
              tick.then(({data}) => {
                if (data && data.code === 0) {
                  this.$message({
                    message: '操作成功',

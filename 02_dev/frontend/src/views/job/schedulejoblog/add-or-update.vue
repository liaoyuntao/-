<template>
  <el-dialog
    ...
    :close-on-click-modal="false" :model="false"
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (id) {
            API.schedulejoblog.info(id).then(({data}) => {
              if (data && data.code === 0) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            var tick = !this.dataForm.id ? API.schedulejoblog.add(params) : API.schedulejoblog.update(params)
            tick.then(({data}) => {
              if (data && data.code === 0) {
                this.$message({
                  message: '操作成功',
